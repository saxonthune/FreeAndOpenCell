      ...
      fc.property(
        ...
        (seed, choices) => {
          ...
          if (isWon(state) && isStuck(state))
            throw new Error(
              'ENG-9: isWon and isStuck both true on initial deal',
            );
          ...
          for (const choice of choices) {
            if (isWon(state)) {
              ...
              if (probe.length > 0) {
                ...
                if (r.ok)
                  throw new Error(
                    'ENG-8: applyAction on won state returned ok',
                  );
              }
              break;
            }
            ...
            if (isStuck(state)) {
              if (legal.length !== 0)
                throw new Error('ENG-9: isStuck but legal.length !== 0');
              if (isWon(state))
                throw new Error('ENG-9: isStuck and isWon both true');
              break;
            }
            ...
            if (!result.ok)
              throw new Error(
                `ENG-5: legal action returned error: ${result.error}`,
              );
            state = result.value;
